...
10. 部署成功后把 SSH、数据库、后台和 APP 路径记录到 `http://部署器域名/ip.html`
...
- **成功记录会落盘**：按业务要求，部署成功后会把 SSH、数据库、后台账号保存到部署器服务器的 `data/deploy-records.json`，并通过 `/ip.html` 查看。这个文件已经加入 `.gitignore`，不会上传到公开仓库。
- **必须启用访问码**：公网部署器建议设置 `ACCESS_CODE`，`/api/deploy-records` 需要访问码才会返回记录。
...
部署成功后访问 `/ip.html` 可查看成功记录；运营版还会读取目标服务器 `/root/wzry-space-install.env`，把自动生成的后台密码和数据库密码同步进记录页。
