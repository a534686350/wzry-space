...
std::string 共享房间ip = MG_SHARE_HOST;//主机  
bool 是否开启共享 = MG_SHARE_ENABLED;
void createSocket() {//重要方法
    ...
    serveraddr.sin_port = htons(MG_SHARE_PORT);
...
int mgSendShareData(const char* data, size_t len) {
    if (!MG_SHARE_ENABLED || data == nullptr || len == 0) {
        return 0;
    }
    if (socket_fd < 0 || ret < 0) {
        createSocket();
    }
    if (socket_fd < 0 || ret < 0) {
        return -1;
    ...
    int sent = send(socket_fd, data, len, flags);
    if (sent < 0 && MG_SEND_RETRY_ON_FAIL) {
        close(socket_fd);
        socket_fd = -1;
        ret = -1;
        createSocket();
        if (socket_fd >= 0 && ret >= 0) {
            sent = send(socket_fd, data, len, flags);
        }
    }
    return sent;
}
