...
function testGameServerWebSocket($host, $port, $timeout = 0.8) {
    $host = normalizeGameServerHost($host);
    $port = (int) $port;
    $started = microtime(true);
    if ($host === '' || $port <= 0 || $port > 65535) {
    ...
    @fwrite($fp, $request);
    $response = '';
    while (!feof($fp) && strlen($response) < 2048) {
        $chunk = @fgets($fp, 512);
    ...
    return ['status' => 'offline', 'ms' => $ms, 'error' => 'WebSocket 握手失败'];
}
...
case 'register':
    ...
    if (isset($card['card_type'])) {
        if ($card['card_type'] === 'day') {
            $expires_at = date('Y-m-d H:i:s', strtotime($bound_at) + 24 * 3600);
        } elseif ($card['card_type'] === 'week') {
            $expires_at = date('Y-m-d H:i:s', strtotime($bound_at) + 7 * 24 * 3600);
        } elseif ($card['card_type'] === 'month') {
            $expires_at = date('Y-m-d H:i:s', strtotime($bound_at) + 30 * 24 * 3600);
        } elseif ($card['card_type'] === 'trial') {
            // 试用卡：自绑定起 5 小时到期
            $expires_at = date('Y-m-d H:i:s', strtotime($bound_at) + 5 * 3600);
        }
    }
    ...
    switch ($action) {
        case 'list':
            try {
                ...
                $stmt->execute($params);
                $list = $stmt->fetchAll();
                $typeNames = ['day' => '天卡', 'week' => '周卡', 'month' => '月卡', 'trial' => '试用卡'];
                ...
                foreach ($list as &$r) {
                    ...
                    $r['card_type_text'] = isset($r['card_type'], $typeNames[$r['card_type']]) ? $typeNames[$r['card_type']] : (empty($r['expires_at']) ? '永久' : '自定义');
        ...
        case 'generate':
            ...
            $typeNames = ['day' => '天卡', 'week' => '周卡', 'month' => '月卡', 'trial' => '试用卡'];
            $typeText = isset($typeNames[$card_type]) ? $typeNames[$card_type] : '';
...
case 'cleanup_expired':
    ...
    if (!$dryRun) {
        adminLog($pdo, '清理过期数据', '删除过期卡密=' . ($stats['expired_cards_deleted'] ?? 0) . ' 删除用户=' . ($stats['users_deleted'] ?? 0));
    }
...
case 'online_users':
    ...
    try {
        ...
        ");
        $stmt = $pdo->prepare('SELECT client, device_id, username, ip, updated_at FROM client_online WHERE updated_at >= ? ORDER BY updated_at DESC LIMIT 200');
        $stmt->execute([$onlineThreshold]);
        apiJsonResp(0, 'ok', ['list' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
    } catch (PDOException $e) {
        apiJsonResp(500, '在线用户加载失败: ' . $e->getMessage());
    }
    break;
...
case 'export':
    ...
    if ($type === 'cards') {
        ...
        foreach ($rows as $r) {
            ...
            $typeText = isset($r['card_type'], $typeNames[$r['card_type']]) ? $typeNames[$r['card_type']] : '';
